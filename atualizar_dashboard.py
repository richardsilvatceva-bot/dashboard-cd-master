...
# ==========================================
GITHUB_TOKEN = "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"
...
def gerar_json():
    ...
    # 4. REPICKING (AGORA 100% DINÂMICO PARA OS FILTROS)
    ...
    if not df_rep.empty:
        ...
        # Agrupamento diário para permitir filtro perfeito no HTML
    ...
    # 6. SOBRAS (INCLUINDO NOVA EXTRAÇÃO DE PRODUTOS E CRUZAMENTO)
    ...
    if not df_sobras.empty:
        # Extração Clássica (Status e Marcas)
        ...
        # Extração Nova (PRODUTO, QTD, VALOR TOTAL)
        df_sob_raw = pd.read_excel(xls, aba_sobras, header=None)
        header_row = -1
        for r in range(min(15, len(df_sob_raw))):
            if any('PRODUTO' in str(v).upper() for v in df_sob_raw.iloc[r].values):
                header_row = r
                break
        if header_row != -1:
            row_vals = [str(v).upper().strip() for v in df_sob_raw.iloc[header_row].values]
            idx_prod = row_vals.index('PRODUTO') if 'PRODUTO' in row_vals else -1
            idx_qtd = next((i for i, v in enumerate(row_vals) if 'QTD' in v and i >= idx_prod), -1)
            idx_val = next((i for i, v in enumerate(row_vals) if 'VALOR' in v and i >= idx_prod), -1)
            
            if idx_prod != -1:
                for r in range(header_row + 1, len(df_sob_raw)):
                    prod = str(df_sob_raw.iloc[r, idx_prod]).strip()
                    if prod.lower() in ['nan', 'none', '', '(vazio)', 'total geral']: continue
                    qtd = tratar_valor_monetario(df_sob_raw.iloc[r, idx_qtd]) if idx_qtd != -1 else 0
                    val = tratar_valor_monetario(df_sob_raw.iloc[r, idx_val]) if idx_val != -1 else 0
                    sobras_detalhe.append({"produto": prod, "qtd": int(qtd), "valor": val})

    # 7. PICLINHA E DIVCIC (COM EXTRAÇÃO DE SALDOS VERMELHOS)
            ...
            if 'SALDO DIVCIC' in row_vals or 'SALDO PICLINHA' in row_vals:
                ...
                if idx_pic != -1:
                    for i in range(r+2, len(df_pic_raw)):
                        ...
                        saldos_piclinha[sku] = int(qtd)
                break

    # CRUZAMENTO SOBRAS x PICLINHA/DIVCIC
    cruzamento_sobras = []
    ...
    try:
        ...
        payload = {
            "message": "Atualização automática (Repicking e Cruzamento Sobras) 🚀", 
