...
# ==========================================
GITHUB_TOKEN = "COLE_O_SEU_TOKEN_AQUI_SEM_ESPACOS"
...
def gerar_json():
    ...
    # 6. SOBRAS (EXTRAÇÃO INTELIGENTE DO NOVO LAYOUT)
    ...
    if not df_sobras.empty:
        # TABELA 1: Ler Status e Marca por Produto
        header_1 = -1
        ...
        if header_1 != -1:
            ...
            for r in range(header_1 + 1, len(df_sobras)):
                ...
                if idx_prod1 != -1:
                    ...
                    if prod.lower() in ['nan', 'none', '', '(vazio)'] or 'TOTAL' in prod.upper():
                        continue
                ...
                # Mapear o SKU e ignorar os Resgatados para a tabela principal de Status
                if prod:
                    sku_map[prod] = {"status": curr_st, "marca": curr_mc}
                
                if curr_st != 'RESGATADO':
                    sobras_dados.append({
                        "status": curr_st, 
                        "marca": curr_mc, 
                        "locacoes": int(loc_val), 
                        "valor": val_val
                    })

        # TABELA 2: Ler as Quantidades (Colunas G a J)
        header_2 = -1
        idx_prod2 = idx_qtd2 = idx_val2 = -1
        for r in range(min(15, len(df_sobras))):
            row_vals = [str(v).upper().strip() for v in df_sobras.iloc[r].values]
            if 'PRODUTO' in row_vals and 'QTD' in row_vals:
                header_2 = r
                for i in range(5, len(row_vals)): # Busca nas colunas à direita
                    if 'PRODUTO' in row_vals[i]: idx_prod2 = i
                    elif 'QTD' in row_vals[i]: idx_qtd2 = i
                    elif 'VALOR' in row_vals[i]: idx_val2 = i
                break
    ...
    try:
        ...
        payload = {
            "message": "Atualização automática (Ignorar Resgatados e Cruzar Status) 🚀", 
